...
export default class TournamentsTable extends React.Component<Props, State> {
    ...
    public renderTableData(): JSX.Element {
        return <>{this.props.tournaments.map(tournament => this.renderTrailRow(tournament))}</>;
    }

    public renderTrailRow(tournament: Tournament): JSX.Element {
        const startDateString = tournament.startDate.toLocaleDateString(undefined, dateDisplayOptions);
        const endDateString = tournament.endDate.toLocaleDateString(undefined, dateDisplayOptions);
        return (
